...
After analyzing the environment it is clear that the current building (*[Figure X](img\1\1_building_visualisation.png)*) blends right in the area. Different kinds of functions that cause multiple identities. The building is a combination of a variation of heights and both monotone and trendy patterned facades. 
...
This is just the first attempt for a better overview, but further possibilities are not excluded and the mapping probably will change during the following design process.
